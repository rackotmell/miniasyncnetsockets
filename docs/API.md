...
- закрывает активные connections;
- снимает event registrations;
- останавливает event loop;
- дожидается завершения event-loop thread, если вызван из внешнего потока;
- только запрашивает остановку и возвращает управление, если вызван из callback
  event loop, поскольку текущий поток нельзя присоединить к самому себе.

Повторный `start()` после `stop()` в `v0.1` не поддерживается. Деструктор
автоматически вызывает `stop()`.
